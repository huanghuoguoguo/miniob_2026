...
### 1.2 问题：为什么数据库要自己管理缓冲池？

既然操作系统有 Page Cache，为什么数据库还要自己实现 Buffer Pool？

**原因一：事务持久性**

数据库需要保证 ACID 特性。如果数据在 Page Cache 中还没刷到磁盘，系统崩溃后数据丢失。数据库需要精确控制**何时刷盘**。

**原因二：预读策略**

数据库知道查询模式，可以预读相关页面。操作系统只能做通用的预读。

**原因三：淘汰策略**

数据库知道页面的重要性（如索引页 vs 数据页）。操作系统只能用通用的 LRU。
...
> **Trade-off 思考**：
> - 用 Page Cache：简单，但控制力弱
> - 自己管理 Buffer Pool：复杂，但可以精细控制
> - 数据库选择后者，因为需要保证数据安全性和性能
...
> **延迟写入（Lazy Writing）**：
> - 修改页面时不立即写磁盘，只标记 dirty
> - 等到淘汰时或显式 flush 时才写磁盘
> - 减少磁盘 I/O 次数
