...
- 这页很容易被学生误解成“数据库和操作系统在抢活干”，所以一定要讲清楚：不是 OS 缓存没价值，而是数据库需要更细粒度、更符合自己语义的控制权
...
- 这页要尽量讲出工程味：缓存淘汰不是随便删，而是要先判断这页现在能不能动、动之前要不要先保存
...
- 要明确告诉他们：调试看 Buffer Pool，不是只看流程走没走到，而是看命中、标脏、pin/unpin、淘汰这些状态有没有按预期变化
